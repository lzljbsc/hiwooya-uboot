...
void OperationSelect(void)
{
	...
	printf("   %c: System Boot system code via Flash(backup firmware).\n", SEL_BOOT_BACKUP_FLASH);
}
...
/*config bootdelay via environment parameter: bootdelay */
	...
	while (timer1 > 0) {
		...
		for (i=0; i<100; ++i) {
			if ((my_tmp = tstc()) != 0) {	/* we got a key press	*/
				...
				BootType = getc();
				if ((BootType < '0' || BootType > '9') && (BootType != 'a'))
					BootType = '3';
				printf("\n\rYou choosed %c\n\n", BootType);
				break;
			}
	...
	if(BootType == '3') {
		...
		char *argv[2];
		sprintf(addr_str, "0x%X", (uint) (gd->bd->bi_flashsize) - 0x0300000 + CFG_FLASH_BASE);
		argv[1] = &addr_str[0];
		printf("   \na: System Boot system code via Flash(backup firmware).\n");
		printf("   boot addr at flash %s\n", addr_str);
		do_bootm(cmdtp, 0, 2, argv);
	} else {
		char *argv[4];
		int argc= 3;
	...
	sprintf(addr_str, "0x%X", (uint) (gd->bd->bi_flashsize) - 0x0300000 + CFG_FLASH_BASE);
	argv[1] = &addr_str[0];
	printf("   \n8: System Boot system code via Flash(backup firmware).\n");
	printf("   boot addr at flash %s\n", addr_str);
	do_bootm(cmdtp, 0, 2, argv);
}
...
{
	u32 val;
	printf("\ngpio init by wooya:www.hi-wooya.com\n");

#if defined(MT7628_ASIC_BOARD)
	/* set GPIO1_MODE */
	val = RALINK_REG(RT2880_GPIOMODE_REG);
	val |= 1 << 14;			//WDT_MODE->KEY
	RALINK_REG(RT2880_GPIOMODE_REG) = val;

	/* set GPIO2_MODE */
	val = 0x1;			//WLED_AN_MODE->GPIO44
	val |= 0x1 << 16;		//WLED_KN_MODE->GPIO35
	RALINK_REG(RT2880_GPIO2MODE_REG) = val;
	
	/* set GPIO32-63 direction */
	val = RALINK_REG(RT2880_REG_PIODIR1);
	val &= ~(0x1 << 6);		//GPIO38 KEY input
	val |= 0x1 << 12;		//GPIO44 led output
	val |= 0x1 << 3;		//GPIO35 led output
	RALINK_REG(RT2880_REG_PIODIR1) = val;

#elif defined(MT7621_ASIC_BOARD)
	/* set GPIO_MODE */
	val = RALINK_REG(RT2880_GPIOMODE_REG);
	val &= ~(0x3 << 8);
	val |= 1 << 8;			//WDT_MODE->KEY
	RALINK_REG(RT2880_GPIOMODE_REG) = val;

	/* set GPIO0-31 direction */
	val = RALINK_REG(RT2880_REG_PIODIR);
	val &= ~(1 << 18);		//GPIO18 KEY input
	...
	val = RALINK_REG(RT2880_GPIOMODE_REG);
	val &= ~(1 << 21);
	val |= (1 << 22);		//WDT_RST_MODE->KEY
	val |= (1 << 9);		//RGMII1_GPIO_MODE->WATCHDOG_EN
	val |= (1 << 13);		//WLED_GPIO_MODE->led
	RALINK_REG(RT2880_GPIOMODE_REG) = val;

	/* set GPIO0-31 direction */
	val = RALINK_REG(RT2880_REG_PIODIR);
	val &= ~(1 << 17);		//GPIO17 KEY input
	val |= 1 << 25;			//GPIO25 WATCHDOG_EN output
	val |= 1 << 24;			//GPIO24 WATCHDOG_WDI output
	RALINK_REG(RT2880_REG_PIODIR) = val;

	/* set GPIO72 direction */
	val = RALINK_REG(RT2880_REG_PIODIR3);
	val |= 1;			//GPIO72 led output
	RALINK_REG(RT2880_REG_PIODIR3) = val;

	/* set GPIO0-31 value */
	RALINK_REG(RT2880_REG_PIOSET) = 1 << 25;	//GPIO25 high
	RALINK_REG(RT2880_REG_PIOSET) = 1 << 24;	//GPIO24 high
...
{
#if defined(MT7628_ASIC_BOARD)
	RALINK_REG(RT2880_REG_PIORESET1) = 1 << 12;	//GPIO44 led on
#elif defined(MT7621_ASIC_BOARD)
#elif defined(MT7620_ASIC_BOARD)
	RALINK_REG(RT2880_REG_PIORESET3) = 1;		//GPIO72 led on
#endif
...
{
#if defined(MT7628_ASIC_BOARD)
	RALINK_REG(RT2880_REG_PIOSET1) = 1 << 12;	//GPIO44 led on
#elif defined(MT7621_ASIC_BOARD)
#elif defined(MT7620_ASIC_BOARD)
	RALINK_REG(RT2880_REG_PIOSET3) = 1;		//GPIO44 led on
#endif
}
...
int detect_wps( void )
{
	u32 val;
#if defined(MT7628_ASIC_BOARD)
	/* get KEY(GPIO38) value */
	val = RALINK_REG(RT2880_REG_PIODATA1);
	val &= (1 << 6);
#elif defined(MT7621_ASIC_BOARD)
        /* get KEY(GPIO18) value */
	val = RALINK_REG(RT2880_REG_PIODATA);
	val &= (1 << 18);
#elif defined(MT7620_ASIC_BOARD)
	/* get KEY(GPIO17) value */
	val = RALINK_REG(RT2880_REG_PIODATA);
	val &= (1 << 17);
#else
	val = 0;
...
{
	u32 val, i;
	u32 agpio_cfg, gpio_mode1, gpio_mode2;
	u32 gpio_ctrl0, gpio_ctrl1, gpio_ctrl2, gpio_ctrl3;
	u32 gpio_data0, gpio_data1, gpio_data2, gpio_data3;
#if defined(MT7628_ASIC_BOARD)
	agpio_cfg = RALINK_REG(RT2880_AGPIOCFG_REG);
	gpio_mode2 = RALINK_REG(RT2880_GPIO2MODE_REG);
#endif
	gpio_mode1 = RALINK_REG(RT2880_GPIOMODE_REG);

	gpio_ctrl0 = RALINK_REG(RT2880_REG_PIODIR);
	gpio_ctrl1 = RALINK_REG(RT2880_REG_PIODIR1);
	gpio_ctrl2 = RALINK_REG(RT2880_REG_PIODIR2);
#if defined(MT7620_ASIC_BOARD)
	gpio_ctrl3 = RALINK_REG(RT2880_REG_PIODIR3);
#endif
	gpio_data0 = RALINK_REG(RT2880_REG_PIODATA);
	gpio_data1 = RALINK_REG(RT2880_REG_PIODATA1);
	gpio_data2 = RALINK_REG(RT2880_REG_PIODATA2);
#if defined(MT7620_ASIC_BOARD)
	gpio_data3 = RALINK_REG(RT2880_REG_PIODATA3);
#endif
	/* set gpio mode */
#if defined(MT7628_ASIC_BOARD)
	/* set agpio_cfg */
	val = 0;
	val |= 0x1;		//I2S_SDI_AIO_EN	Digital PAD
	val |= 0x1 << 1;	//I2S_SDO_AIO_EN	Digital PAD
	val |= 0x1 << 2;	//I2S_WS_AIO_EN		Digital PAD
	val |= 0x1 << 3;	//I2S_CLK_AIO_EN	Digital PAD
	val |= 0x1 << 4;	//PEF_CLKO_AIO_EN	Digital PAD
	val |= 0xf << 17;	//EPHY_GPIO_AIO_EN	Digital PAD
	RALINK_REG(RT2880_AGPIOCFG_REG) = val;
	/* set gpio_mode2 */
	val = 0;
	val |= 0x1;		//WLED_AN_MODE
	val |= 0x1 << 2;	//P0_LED_AN_MODE
	val |= 0x1 << 4;	//P1_LED_AN_MODE
	val |= 0x1 << 6;	//P2_LED_AN_MODE
	val |= 0x1 << 8;	//P3_LED_AN_MODE
	val |= 0x1 << 10;	//P4_LED_AN_MODE
	//val |= 0x1 << 16;	//WLED_KN_MODE
	//val |= 0x1 << 18;	//P0_LED_KN_MODE
	//val |= 0x1 << 20;	//P1_LED_KN_MODE
	//val |= 0x1 << 22;	//P2_LED_KN_MODE
	//val |= 0x1 << 24;	//P3_LED_KN_MODE
	//val |= 0x1 << 26;	//P4_LED_KN_MODE
	RALINK_REG(RT2880_GPIO2MODE_REG) = val;
	/* set gpio_mode1 */
	val = 0;
	//val |= 0x1;		//GPIO_MODE
	val |= 0x1 << 2;	//SPIS_MODE
	val |= 0x1 << 4;	//SPI_CS1_MODE
	val |= 0x1 << 6;	//I2S_MODE
	//val |= 0x1 << 8;	//UART0_MODE
	val |= 0x1 << 10;	//SD_MODE
	//val |= 0x1 << 12;	//SPI_MODE
	val |= 0x1 << 14;	//WDT_MODE
	//val |= 0x1 << 16;	//PREST_MODE
	val |= 0x1 << 18;	//REFCLK_MODE
	val |= 0x1 << 20;	//I2C_MODE
	val |= 0x1 << 24;	//UART1_MODE
	val |= 0x1 << 26;	//UART2_MODE
	val |= 0x1 << 28;	//PWM0_MODE
	val |= 0x1 << 30;	//PWM1_MODE
	RALINK_REG(RT2880_GPIOMODE_REG) = val;
#elif defined(MT7621_ASIC_BOARD)
	val = 0;
	//val |= 0x1 << 1;	//UART1_MODE
	val |= 0x1 << 2;	//I2C_MODE
	val |= 0x1 << 3;	//UART3_MODE
	val |= 0x1 << 5;	//UART2_MODE
	val |= 0x1 << 7;	//JTAG_MODE
	val |= 0x1 << 8;	//WDT_MODE
	val |= 0x1 << 10;	//PERST_MODE
	val |= 0x1 << 12;	//MDIO_MODE
	val |= 0x1 << 14;	//RGMII1_MODE
	val |= 0x1 << 15;	//RGMII2_MODE
	val |= 0x1 << 16;	//SPI_MODE
	val |= 0x1 << 18;	//SDXC_MODE
	val |= 0x1 << 20;	//ESWINT_MODE
	RALINK_REG(RT2880_GPIOMODE_REG) = val;
#elif defined(MT7620_ASIC_BOARD)
	val = 0;
	val |= 1;		//I2C_GPIO_MODE
	val |= 0x7 << 2;	//UARTF_SHARE_MODE
	//val |= 0x1 << 5;	//UARTL_GPIO_MODE
	val |= 0x2 << 7;	//MDIO_GPIO_MODE
	val |= 0x1 << 9;	//RGMII1_GPIO_MODE
	val |= 0x1 << 10;	//RGMII2_GPIO_MODE
	val |= 0x1 << 11;	//SPI_GPIO_MODE
	val |= 0x1 << 12;	//SPI_REFCLK0_MODE
	val |= 0x1 << 13;	//WLED_GPIO_MODE
	val |= 0x1 << 15;	//EPHY_LED_GPIO_MODE
	val |= 0x2 << 16;	//PREST_GPIO_MODE
	val |= 0x2 << 18;	//ND_SD_GPIO_MODE
	val |= 0x1 << 20;	//PA_G_GPIO_MODE
	val |= 0x2 << 21;	//WDT_RST_MODE
	val |= 0x1 << 30;	//SUTIF_SHARE_MODE
	RALINK_REG(RT2880_GPIOMODE_REG) = val;
#endif
	...
	RALINK_REG(RT2880_REG_PIODIR) = val;
	RALINK_REG(RT2880_REG_PIODIR1) = val;
	RALINK_REG(RT2880_REG_PIODIR2) = val;
#if defined(MT7620_ASIC_BOARD)
	RALINK_REG(RT2880_REG_PIODIR3) = val;
#endif
	...
	/* set KEY as input */
#if defined(MT7628_ASIC_BOARD)
	RALINK_REG(RT2880_REG_PIODIR1) &= ~(1 << 6);	//GPIO38
#elif defined(MT7621_ASIC_BOARD)
	RALINK_REG(RT2880_REG_PIODIR) &= ~(1 << 18);	//GPIO18
#elif defined(MT7620_ASIC_BOARD)
	RALINK_REG(RT2880_REG_PIODIR) &= ~(1 << 17);	//GPIO17
#endif
	...
	for (i = 0; i < 100; i++) {
		...
		RALINK_REG(RT2880_REG_PIODATA1) = val;
		RALINK_REG(RT2880_REG_PIODATA2) = val;
#if defined(MT7620_ASIC_BOARD)
		RALINK_REG(RT2880_REG_PIODATA3) = val;
#endif
		udelay(200000);
		printf("\nall led off\n");
		val = 0x0;
	...
	/* test exit */
#if defined(MT7628_ASIC_BOARD)
	RALINK_REG(RT2880_AGPIOCFG_REG) = agpio_cfg;
	RALINK_REG(RT2880_GPIO2MODE_REG) = gpio_mode2;
#endif
	RALINK_REG(RT2880_GPIOMODE_REG) = gpio_mode1;

	RALINK_REG(RT2880_REG_PIODIR) = gpio_ctrl0;
	RALINK_REG(RT2880_REG_PIODIR1) = gpio_ctrl1;
	RALINK_REG(RT2880_REG_PIODIR2) = gpio_ctrl2;

	RALINK_REG(RT2880_REG_PIODATA) = gpio_data0;
	RALINK_REG(RT2880_REG_PIODATA1) = gpio_data1;
	RALINK_REG(RT2880_REG_PIODATA2) = gpio_data2;
#if defined(MT7620_ASIC_BOARD)
	RALINK_REG(RT2880_REG_PIODIR3) = gpio_ctrl3;
	RALINK_REG(RT2880_REG_PIODATA3) = gpio_data3;
#endif
}
