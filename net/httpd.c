...
int do_http_upgrade( const ulong size, const int upgrade_type ){
	...
	} else if (upgrade_type == WEBFAILSAFE_UPGRADE_TYPE_BACKUP) {
		printf( "\n\n******************************\n* BACKUP FIRMWART UPGRADING  *\n* DO NOT POWER OFF DEVICE!   *\n******************************\n\n" );
		DECLARE_GLOBAL_DATA_PTR;
		printf("addr:0x%x, size:%d\n", (uint) (gd->bd->bi_flashsize) - 0x0300000, size);
		return( raspi_erase_write( ( u8_t * )WEBFAILSAFE_UPLOAD_RAM_ADDRESS,(uint) (gd->bd->bi_flashsize) - 0x0300000, size ) );
	} else {
		return(-1);
	}
