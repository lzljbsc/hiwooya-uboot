...
/* #if !defined(WEBFAILSAFE_UPLOAD_ART_ADDRESS)
					...
					end = (char *)strstr((char *)start, "name=\"backup\"");
					if (end) {
						printf("Upgrade type: BACKUP FIRMWARE\n");
						webfailsafe_upgrade_type = WEBFAILSAFE_UPGRADE_TYPE_BACKUP;
					} else {
...
*/
				// backup fimware
				} else if ((webfailsafe_upgrade_type) == WEBFAILSAFE_UPGRADE_TYPE_BACKUP && (hs->upload_total > WEBFAILSAFE_UPLOAD_BACKUP_SIZE_IN_BYTES)) {
				printf("## Error: wrong file size, should be: %d bytes!\n", WEBFAILSAFE_UPLOAD_BACKUP_SIZE_IN_BYTES);
				webfailsafe_upload_failed = 1;
